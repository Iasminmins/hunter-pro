...
import { csvHeaderError, parseCsv, summarizeTrades, validateMonthlyInput } from './model.mjs';
...
async function submitMonth(event) {
  ...
  try {
    ...
    const ninjaLines = ninjaText.replace(/^\uFEFF/, '').trim().split(/\r?\n/).filter(line => line.trim());
    const ninjaError = ninjaLines.length < 2 || !ninjaLines[0].includes(ninjaText.includes(';') ? ';' : ',') ? 'CSV NinjaTrader / Grid: verifique o cabeçalho e inclua pelo menos uma linha.' : '';
    ...
    const staged = { id:crypto.randomUUID(), month, year, hunterVersion:String(data.hunterVersion).trim(), hsgConfig:String(data.hsgConfig).trim(), observations:String(data.observations || '').trim(), status:'processed', createdAt:new Date().toISOString(), importedAt:new Date().toISOString(), summary, trades, ninjaRows:ninjaLines.length - 1, revision:1 };
    ...
    commitImport(staged); closeModal(); render(); showToast(`${months[month-1]} ${year} importado · ${trades.length} linhas processadas.`);
