...
function renderGeneral(state) {
  ...
  const allTrades = tradesOf(state);
  ...
  const imported = state.months.length ? table(['Mês','Versão','HSG trades','Grid trades','Acerto','PF','Resultado R','Status'], months.slice().reverse().map(block => `<tr><td><strong>${monthNames[block.month-1]} ${block.year}</strong></td><td>${esc(block.hunterVersion)}</td><td>${block.summary.trades}</td><td>${block.ninjaRows ?? '—'}</td><td>${num(block.summary.winRate*100)}%</td><td>${fmtPF(block.summary.profitFactor)}</td><td>${fmtR(block.summary.totalR)}</td><td>Importado</td></tr>`).join('')) : empty('Nenhum fechamento mensal importado.','Adicione um mês na seção Meses.');
...
function renderMonths(state) {
  const years = [...new Set([new Date().getFullYear(), state.selectedMonthYear, ...state.months.map(m => m.year)].filter(Boolean))].sort((a,b) => b-a);
  ...
  const cards = monthNames.map((name,index) => {
    ...
    return `<article class="month-card ${block ? 'has-data' : ''}"><span>${name}</span>${block ? `<strong>${block.summary.trades} trades · ${fmtR(block.summary.totalR)}</strong><small>${esc(block.hunterVersion)} · ${block.revision ? `rev. ${block.revision}` : 'importado'}</small>` : '<small>Sem dados</small>'}<button class="button ${block ? 'secondary' : 'primary'}" data-action="create-month" data-month="${index+1}" data-year="${year}">${block ? 'Atualizar mês' : `Importar ${name}`}</button></article>`;
  }).join('');
  return `<section class="panel"><div class="panel-heading"><div><h2>Regime recente ponderado</h2><p class="panel-description">Até três fechamentos mais recentes · pesos 50%, 30% e 20%, normalizados se faltarem meses.</p></div><label>Ano <select id="month-year">${years.map(y => `<option value="${y}" ${y===year?'selected':''}>${y}</option>`).join('')}</select></label></div>${recent.months.length ? table(['Mês','Peso','Trades','Acerto','PF','Resultado'],recent.weightsUsed.map(item => { const block=recent.months.find(m => m.month===item.month&&m.year===item.year); return `<tr><td>${monthNames[item.month-1]} ${item.year}</td><td>${num(item.weight*100)}%</td><td>${block.summary.trades}</td><td>${num(block.summary.winRate*100)}%</td><td>${fmtPF(block.summary.profitFactor)}</td><td>${fmtR(block.summary.totalR)}</td></tr>`; }).join('')) : empty('Nenhum mês importado.','O regime será calculado após os fechamentos.')}</section><section class="panel"><div class="panel-heading"><h2>Calendário de ${year}</h2><span class="panel-tag">${state.months.filter(m=>m.year===year).length} mês(es) preservado(s)</span></div><div class="month-grid">${cards}</div></section>`;
}
